...
def remove_duplicates(lilist):
    current1 = lilist.head
    while current1:
...
def find_union(lilist1, lilist2):
    if lilist1.head is None:
        union = lilist2.duplicate()
        remove_duplicates(union)
        return union
    if lilist2.head is None:
        union = lilist1.duplicate()
        remove_duplicates(union)
        return union
    ...
    lilist2_copy = lilist2.duplicate()
    last_node.next = lilist2_copy.head
    remove_duplicates(union)
 
    return union
 
 
def find_intersection(lilist1, lilist2):
    if (lilist1.head is None or lilist2.head is None):
        return LinkedList()
...
a_lilist1 = LinkedList()
a_lilist2 = LinkedList()
data_list = input('Please enter the elements in the first linked list: ').split()
for data in data_list:
    node = Node(int(data))
    a_lilist1.insert_at_end(node)
data_list = input('Please enter the elements in the second linked list: ').split()
for data in data_list:
    node = Node(int(data))
    a_lilist2.insert_at_end(node)
 
union = find_union(a_lilist1, a_lilist2)
intersection = find_intersection(a_lilist1, a_lilist2)
